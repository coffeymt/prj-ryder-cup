...
## Overview
This document enumerates every Cloudflare resource, Workers secret, DNS record, and third-party account the Ryder Cup web app requires across local, preview, and production environments. It is the canonical reference for provisioning, post-deploy verification, and the user-facing setup runbook (`assets/SETUP.md`). Product context and the rationale behind the Cloudflare + D1 + smtp2go stack live in `plan/ryder-cup-app/prd.md` under **Hosting & Infrastructure**.
...
- **Project name:** `rydercup` — single source of truth. Configured in `web/wrangler.jsonc` as the `name` field; downstream tooling reads it from there.
...
- **Default Pages URL:** `https://rydercup.pages.dev` (production). Preview branches deploy to `<branch>.rydercup.pages.dev`.
...
- **Provisioned databases** (IDs recorded in `web/wrangler.jsonc`):
  ...
  |---|---|---|
  | preview | `rydercup-preview` | `bc4be116-2576-4517-a952-5e35607d752d` |
  | production | `rydercup-prod` | `ab56b1d2-b5b1-4652-8d9e-6ad0a5912461` |
...
## Custom Domain & DNS
- **Target hostname:** `rydercup.sbcctears.com` — **live** and serving the production deployment. Also reachable at `https://rydercup.pages.dev`.
- **DNS record (present on the `sbcctears.com` zone):**
  - Type: `CNAME`
  - Name: `rydercup`
  - Target: `rydercup.pages.dev`
...
- **Verify command** (PowerShell-friendly, since the workspace OS is Windows):
  ...
  Expected output resolves through Cloudflare's edge to `rydercup.pages.dev`.
...
**Registration reference (Pages Functions use `pages secret`, not `secret`):**
    ...
    wrangler pages secret put <NAME> --project-name rydercup
...
**Verify command** (lists secret names, never values):
    ...
    wrangler pages secret list --project-name rydercup
...
| `preview` | Every non-`main` branch; PR review deploys. | `rydercup-preview` | `<branch>.rydercup.pages.dev` |
| `production` | `main` branch auto-deploys. | `rydercup-prod` | `rydercup.sbcctears.com` (also `rydercup.pages.dev`) |
...
## Applied Migrations
Migrations live in `web/migrations/` and are applied in filename order to each D1 database. As of the current deploy, the following are applied to **both `rydercup-preview` and `rydercup-prod`**:
...
| Pages project exists | `wrangler pages project list` (expect `rydercup`) |
| Production deployment live | `wrangler pages deployment list --project-name rydercup` |
| Preview D1 exists | `wrangler d1 list` (expect `rydercup-preview`) |
| Production D1 exists | `wrangler d1 list` (expect `rydercup-prod`) |
...
| Preview secrets registered | `wrangler pages secret list --project-name rydercup` (preview env — expect all five names) |
| Production secrets registered | `wrangler pages secret list --project-name rydercup` (production env — expect all five names) |
| DNS — custom domain CNAME | `nslookup -type=CNAME rydercup.sbcctears.com` (expect `rydercup.pages.dev`) |
| TLS — app reachable | `curl -I https://rydercup.sbcctears.com` (expect `200`, cert issued by Google Trust Services) |
