...
<main class="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center px-6 py-10">
  <section class="border-border bg-surface p-card-padding space-y-8 rounded-3xl border shadow-sm">
    ...
    <form method="POST" class="space-y-5">
      <div class="space-y-2">
        ...
        <input
          ...
          class="min-h-touch border-border font-display text-text-primary focus:border-accent focus:ring-accent w-full rounded-lg border bg-[var(--color-bg)] px-4 text-center text-3xl font-semibold tracking-[0.28em] uppercase transition outline-none focus:ring-2"
          placeholder="ABC123"
          required
        />
      </div>
      ...
      <button
        type="submit"
        class="min-h-touch bg-accent text-accent-text hover:bg-accent-hover focus-visible:outline-accent flex w-full items-center justify-center rounded-lg px-6 text-base font-semibold transition focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2"
      >
        Join Tournament
      </button>
    </form>
