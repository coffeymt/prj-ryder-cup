      ...
      <form method="POST" use:enhance={signInEnhance} class="mt-6 space-y-4">
        <div>
          ...
          <input
            ...
            autofocus
            value={form?.email ?? ''}
            class="min-h-touch border-border bg-bg text-text-primary focus:border-accent focus:ring-accent-ring mt-2 w-full rounded-lg border px-4 text-base transition outline-none focus:ring-1"
          />
          {#if displayedError}
        ...
        <button
          type="submit"
          class="min-h-touch bg-accent text-accent-text hover:bg-accent-hover inline-flex w-full items-center justify-center gap-2 rounded-xl px-4 text-base font-semibold shadow-md transition hover:shadow-lg disabled:cursor-not-allowed disabled:opacity-50"
