...
export const PATCH: RequestHandler = async (event) => {
  ...
  if (body.pointsToWin !== undefined) {
    if (
      typeof body.pointsToWin !== 'number' ||
      !Number.isFinite(body.pointsToWin) ||
      body.pointsToWin <= 0
    ) {
  ...
  if (body.allowanceScrambleLow !== undefined) {
    updates.allowance_scramble_low = readOptionalRatio(
      body.allowanceScrambleLow,
      'allowanceScrambleLow'
    );
  }

  if (body.allowanceScrambleHigh !== undefined) {
    updates.allowance_scramble_high = readOptionalRatio(
      body.allowanceScrambleHigh,
